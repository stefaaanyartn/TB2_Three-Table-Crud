...
public class StudentQueryImplementation implements QueryContract.StudentQuery {
    ...
    private Student getStudentFromCursor(Cursor cursor){
        @SuppressLint("Range") int id = cursor.getInt(cursor.getColumnIndex(STUDENT_ID));
        @SuppressLint("Range") String name = cursor.getString(cursor.getColumnIndex(STUDENT_NAME));
        @SuppressLint("Range") int regNum = cursor.getInt(cursor.getColumnIndex(STUDENT_REGISTRATION_NUM));
